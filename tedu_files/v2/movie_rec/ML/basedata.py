...
def init_item_item_simmat():
    ...
    ITEM_ITEM_SIMMAT = pd.DataFrame(
        0.0, index=itemids, columns=itemids, dtype='f8')
    """
    ...
    MOVIE_VEC_COLUMNS = pd.unique(
        np.hstack((unique_directors, unique_actors, unique_genres, unique_languages)))
...
if not os.path.exists('basedata/ITEM_ITEM_SIMMAT.csv'):
    ...
    ITEM_ITEM_SIMMAT = pd.read_csv(
        'basedata/ITEM_ITEM_SIMMAT.csv', index_col=0, header=0)
